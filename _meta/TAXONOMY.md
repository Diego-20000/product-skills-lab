...
- **`integrations/`**: **consumir** APIs de terceros — evaluar proveedores, resiliencia ante su downtime, gestión de claves, caché, rate limits. La distinción con `server/`: si construís el endpoint, es `server/`; si lo consumís de alguien más, es `integrations/`. No entran catálogos de APIs copiados de listas existentes (ver el criterio general): un directorio de APIs se referencia como fuente, no se reproduce.
